...
- Paper 账本内核可验证 append-only 哈希链、费用、成交、未成交、持仓和现金重放；当前每决策点的 signal/model/source 哈希仍由调用者提供，且缺日频NAV/回撤盯市，因此准入固定 `blocked_missing_controlled_paper_signal_adapter` 与 `blocked_missing_daily_paper_risk_marks`，更不代表收益回测、券商授权或真实成交。
