...
- `technical_alpha_feasibility_report.v1.json`：只允许Development/Validation的base/stress 16项指标及四种终态（含严格包络失败的`BLOCKED_ADAPTER_PROTOCOL`）；数值指标用canonical decimal string避免门槛舍入，并绑定plan、PIT/history manifest、实验、引擎和Gate源码哈希；任何blocked终态的指标必须为`null`，Locked Test和全部执行权限固定关闭。
