...
本目录提供 1 万元 ETF 低频策略的 Paper 执行内核。当前状态仍是 `paper_only`，不会提交真实订单。现在另有华泰 MQuant 的只读 Shadow 适配器，但本机尚未安装或获权使用官方客户端，因此不能声称真实账户已经连通。
...
- `risk.py`：Paper / Shadow / Live fail-closed 门禁，并签发绑定计划与账户快照的短时批准；
...
`huatai_shadow_probe` 当前应返回 `blocked`，直到用户通过华泰正式渠道开通并安装 MATIC/MQuant、导入 `integrations/htsc_mquant/htsc_shadow_exporter.py` 并产生完整快照。读到一次快照不等于获准实盘交易。
